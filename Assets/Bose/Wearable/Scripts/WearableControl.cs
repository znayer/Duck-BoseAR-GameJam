...
namespace Bose.Wearable
{
	...
	public sealed class WearableControl : Singleton<WearableControl>
	{
			...
			float autoReconnectTimeout = WearableConstants.DefaultAutoReconnectTimeout)
		{
			...
			var safeAutoReconnectTimeout = Mathf.Max(0, autoReconnectTimeout);
			var doAutoReconnect = autoReconnect && safeAutoReconnectTimeout > 0;

			if (autoReconnect && !doAutoReconnect)
			{
				Debug.LogWarningFormat(WearableConstants.InvalidAutoReconnectTimeoutPeriodWarning, autoReconnectTimeout);
			}
		...
		private WearableProviderBase GetOrCreateProvider(ProviderId providerId)
		{
			switch (providerId)
			{
				...
				case ProviderId.WearableProxy:
					Debug.LogWarning(WearableConstants.ProxyProviderCreateWarning);
					if (_debugProvider == null)
					{
						_debugProvider = new WearableDebugProvider();
					}
		...
		{
			// Convert mobile and proxy providers to debug provider
			const string RECORD_MESSAGE = "UpdatedProvider";

			#pragma warning disable 618
			if (_editorDefaultProvider == ProviderId.MobileProvider ||
			    _editorDefaultProvider == ProviderId.WearableProxy)
			{
				UnityEditor.Undo.RecordObject(this, RECORD_MESSAGE);

				Debug.LogWarning(
					_editorDefaultProvider == ProviderId.MobileProvider ?
						WearableConstants.MobileProviderRemovedWarning :
						WearableConstants.ProxyProviderRemovedWarning);

				_editorDefaultProvider = ProviderId.DebugProvider;
			}

			if (_runtimeDefaultProvider == ProviderId.MobileProvider ||
			    _runtimeDefaultProvider == ProviderId.WearableProxy)
			{
				UnityEditor.Undo.RecordObject(this, RECORD_MESSAGE);
