...
namespace Bose.Wearable
{
	...
	public sealed class WearableDebugProvider : WearableProviderBase
	{
		...
		public void SimulateGesture(GestureId gesture)
		{
			...
			{
				Debug.LogError(WearableConstants.NoneIsInvalidGesture);
			}
			else
			{
		...
		protected override void SetActiveNoiseReductionModeInternal(ActiveNoiseReductionMode mode)
		{
			...
			{
				Debug.LogWarning(WearableConstants.DebugProviderAnrNotEnabledWarning);
			...
			if (_virtualDevice.IsActiveNoiseReductionModeAvailable(mode))
			{
				...
				if (_verbose)
				{
					Debug.LogFormat(WearableConstants.DebugProviderSetAnrModeFormat, mode.ToString());
				}
			}
			else
			{
				Debug.LogWarningFormat(WearableConstants.DebugProviderSetInvalidAnrModeWarning, mode.ToString());
			}
		...
		protected override void SetControllableNoiseCancellationLevelInternal(int level, bool enabled)
		{
			...
			{
				Debug.LogWarning(WearableConstants.DebugProviderCncNotEnabledWarning);
			...
			{
				Debug.LogFormat(
					WearableConstants.DebugProviderSetCncLevelFormat,
		...
		internal WearableDebugProvider()
		{
			...
			_dynamicDeviceInfo = new DynamicDeviceInfo
			{
				...
				activeNoiseReductionMode = WearableConstants.DebugProviderDefaultAnrMode,
				availableActiveNoiseReductionModes = WearableTools.GetActiveNoiseReductionModesAsInt(
					WearableConstants.DebugProviderDefaultAvailableAnrModes),
				controllableNoiseCancellationLevel = WearableConstants.DebugProviderDefaultCncLevel,
				controllableNoiseCancellationEnabled = WearableConstants.DebugProviderDefaultCncEnabled,
				totalControllableNoiseCancellationLevels = WearableConstants.DebugProviderDefaultTotalCncLevels
			};
		...
		private void PerformDeviceConnection()
		{
			...
			switch (_connectionPhase)
			{
				...
				case ConnectionPhase.GenerateIntentsResponse:
					...
					if (_connectionIntentProfile == null && _verbose)
					{
